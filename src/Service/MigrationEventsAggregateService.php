...
namespace BlackCat\Database\Packages\MigrationEvents\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\MigrationEvents\Dto\MigrationEventDto;
use BlackCat\Database\Packages\MigrationEvents\Mapper\MigrationEventDtoMapper;
 ...
 */
final class MigrationEventsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private MigrationEventRepository $migrationEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'migration_events' => $this->migrationEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\MigrationEvents\ModuleException("Record {$idStr} not found for locking.");
            }
