...
namespace BlackCat\Database\Packages\MigrationEvents;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\MigrationEvents\Criteria;
use BlackCat\Database\Packages\MigrationEvents\Repository\MigrationEventRepositoryInterface;
use BlackCat\Database\Packages\MigrationEvents\Repository\MigrationEventRepository as GeneratedRepository;
...
{
  private MigrationEventRepositoryInterface $repo;
  ...
  public function __construct(private Database $db)
  {
    ...
    public function _setRepositoryForTests(RepoContract $repo): void
    {
        ...
        if (!$repo instanceof MigrationEventRepositoryInterface) {
            // Allow any repo that conforms to the interface even if not generated
            throw new \InvalidArgumentException('Expected MigrationEventRepositoryInterface');
        }
        $this->repo = $repo;
    }
    ...
    public function getByUnique(array $keys, bool $asDto = false): array|\BlackCat\Database\Packages\MigrationEvents\Dto\MigrationEventDto|null { return $this->repo->getByUnique($keys, $asDto); }
    public function getById(int|string|array $id, bool $asDto = false): array|\BlackCat\Database\Packages\MigrationEvents\Dto\MigrationEventDto|null { return $this->repo->getById($id, $asDto); }
